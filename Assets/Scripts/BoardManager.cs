...
public class BoardManager : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        // Accept either mouse click or pinch
        bool clicked = Input.GetMouseButtonDown(0) || HandReceiver.PinchDown();

        if (clicked && selectionX >= 0 && selectionY >= 0)
        {
    ...
    private void UpdateSelection()
    {
        ...
        // Use thumb position from hand tracking instead of mouse
        Vector3 screenPos = new Vector3(
            HandReceiver.thumbPos.x * Screen.width,
            (1 - HandReceiver.thumbPos.y) * Screen.height,
            0
        );

        Ray ray = Camera.main.ScreenPointToRay(screenPos);
        if (ray.direction.y >= 0) return; // ray pointing away from board
